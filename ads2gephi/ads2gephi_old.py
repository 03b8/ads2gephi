...
class CitationNetwork(object):
    ...
    def author_is_same(node1, node2):
        """
        Check if first author is the same in two nodes
    ...
    def smp_snowball(nodelist, start_year, end_year, scope):
        ...
        if scope == 'cit':
            ...
            for node in nodelist:
                if node.citation_nodes is not None:
                    for bc in node.citation_nodes.split('; '):
            ...
            print('Initializing sampler...')
            for node in nodelist:
                if node.reference_nodes is not None:
                    for bc in node.reference_nodes.split('; '):
                        print('Checking bibcode {}'.format(bc))
                        if start_year < int(bc[0:4]) < end_year:
                            print('Querying ADS...')
                            for i in range(5):
    ...
    def make_regular_edges(self, direction):
        ...
        for node in self.nodelist:
            ...
            if node.citation_nodes is not None:
                for citation_bc in node.citation_nodes.split('; '):
            ...
            if node.reference_nodes is not None:
                for reference_bc in node.reference_nodes.split('; '):
...
class Node(object):
    ...
    def __init__(self, ads_obj):
        ...
        self.citation = ads_obj.citation_nodes
        self.reference = ads_obj.reference_nodes
